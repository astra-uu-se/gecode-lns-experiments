...
class Constraint:
    ...
    def strength(self):
      ...
      if self.identifier in {'le', 'lt', 'neq', 'gt', 'ge'}:
          return 0
      if self.identifier in {'increasing', 'decreasing', 'strictly_increasing', 'strictly_decreasing'}:
          return 10
      if self.identifier in {'global_cardinality_closed', 'global_cardinality_low_up_closed'}:
          return 20
...
if __name__ == '__main__':
    ...
    if problem == 'smsd':
        ...
        graph.constraints.add(Constraint('bin_packing_load', set(placed_in), set(load)))
        for s in range(num_slabs):
            for c in range(num_colors):
                graph.constraints.add(
                  Constraint('bool2int exists', set(placed_in), {has_color[s][c]}))
        for s in range(num_slabs):
            graph.constraints.add(Constraint('sum', set(has_color[s]), {n_colors[s]}))
        graph.constraints.add(Constraint('sum element', set(load), {objective}))
        ...
        for n in range(num_nodes):
            graph.constraints.add(
              Constraint('element', set(departure + [pred[n]]), {departure_pred[n]}))
        for n in range(num_nodes):
            graph.constraints.add(
              Constraint('sum', {departure_pred[n], dur_from_pred[n]}, {arrival[n]}))
        for n in range(1, num_nodes):
            graph.constraints.add(
              Constraint('max', {arrival[n], departure[n - 1]}, set(), Variable(f'viol_max[n]')))
        ...
        for n in range(num_nodes):
            graph.constraints.add(
              Constraint('element', set(tvar).union({prev[n]}), {tvar_element[n]}))
        ...
        for car in range(num_cars):
            graph.constraints.add(
              Constraint('element', {car_class[car]}, set(car_has_feature[car])))
        graph.constraints.add(
        ...
        for n in range(num_nurses):
            graph.constraints.add(
                Constraint('regular',
                          {roster[n][d] for d in range(num_days)},
                          set()))
